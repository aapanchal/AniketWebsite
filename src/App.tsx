...
const coderData = {
  ...
  skills: [
    ...
    'TailwindCSS', 'CSS', 'SCSS', 'Figma', 'GitHub', 'HTML', 
    'Node.js', 'Express', 'MongoDB', 'Firebase', 'Docker', 'Git', 'Storybook'
  ],
  tags: [
    'MARN Stack', 'Clean Code', 'Innovation', 'TypeScript Enthusiast', 
      ...
      <div className="overflow-hidden border-t-[2px] border-zinc-300 dark:border-indigo-900 px-4 lg:px-8 py-4 lg:py-8 relative">
        ...
        <div className="relative flex">
          ...
          <code className="font-mono text-xs md:text-sm lg:text-base w-full">
            ...
            <div className="pl-6">
              <span className="text-zinc-800 dark:text-white">tags:</span>
              <span className="text-zinc-600 dark:text-gray-400">{'['}</span>
              <div className="pl-6 flex flex-wrap">
                {coderData.tags.map((tag, index) => (
                  <span key={tag} className="mr-1">
                    <span className="text-zinc-600 dark:text-gray-400">'</span>
                    <span className="text-orange-600 dark:text-orange-400">{tag}</span>
                    <span className="text-zinc-600 dark:text-gray-400">'</span>
                    {index < coderData.tags.length - 1 && (
                      <span className="text-zinc-600 dark:text-gray-400">, </span>
                    )}
                  </span>
                ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 xl:gap-16 items-center">
          ...
          <div className="flex flex-col gap-4 sm:gap-6 items-start text-left order-2 lg:order-1 animate-fade-in-up">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3 my-2 sm:my-4">
              {coderData.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 sm:px-4 py-1 sm:py-2 bg-gray-900/80 dark:bg-white/10 border border-gray-700 dark:border-gray-600 rounded-full text-gray-200 dark:text-gray-300 text-sm sm:text-base backdrop-blur-sm hover:bg-gray-800 dark:hover:bg-white/20 transition-all duration-300 cursor-default"
                >
                  {tag}
                </span>
              ))}
            ...
            <p className="text-gray-600 dark:text-gray-300 text-base sm:text-lg lg:text-xl max-w-lg leading-relaxed">
              JavaScript lover 🖋️ | Crafting frameworks and coding the future ✨ | Passionate about building responsive, accessible, and scalable web applications
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-4 sm:mt-6 w-full sm:w-auto">
              ...
              <a
                href="https://drive.google.com/file/d/1zPhFeAe_-NBQnyu1Q_MZ0B79UOfthRMC/view?usp=drive_link"
                target="_blank"
                rel="noopener noreferrer"
                className="px-6 py-3 bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg font-semibold transition-all duration-300 hover:scale-105 active:scale-95 inline-block text-center"
              >
                Get Resume
              </a>
            </div>
